...
export function protectionPlain(p) {
  return [p.name, (p.aliases ?? []).join(" "), "защита от DDoS анти-ддос", p.summary, p.ru_access?.note, ...["pros", "cons", "prices"].flatMap((k) => (p[k] ?? []).map((x) => x.text)), ...(p.timeline ?? []).map((x) => `${x.date} ${x.text}`)]
    .filter(Boolean)
    .join("\n");
}
